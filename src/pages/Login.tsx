      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-xl p-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-gray-900">Welcome to EduAbroad</h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button type="submit" disabled={submitting} className="w-full bg-blue-600 text-white py-3 rounded-xl font-semibold hover:bg-blue-700 transition-colors text-sm disabled:opacity-60 disabled:cursor-not-allowed">
          ...
          <div className="mt-8">
            ...
            <div className="space-y-3">
              ...
              <div>
                <p className="text-xs font-semibold text-blue-600 mb-2">Students</p>
                <div className="grid grid-cols-1 gap-2">
                  {demoAccounts.students.map(a => (
                    <button key={a.email} type="button" onClick={() => loginAs(a.email, a.password)} disabled={submitting}
                      className="flex items-center gap-3 p-3 border border-blue-200 rounded-lg hover:border-blue-400 hover:bg-blue-50 transition-all text-left disabled:opacity-60 disabled:cursor-not-allowed">
                      <div className="w-8 h-8 bg-blue-100 rounded-full flex items-center justify-center text-blue-700 font-bold text-xs flex-shrink-0">{a.initials}</div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-semibold text-gray-800">{a.name}</p>
                        <p className="text-xs text-gray-400 truncate">{a.email}</p>
                      </div>
                      <span className="text-xs text-blue-600 font-medium whitespace-nowrap">Click to login →</span>
                    </button>
                  ))}
                </div>
              </div>
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            ...
            <Link to="/" className="text-blue-600 font-medium hover:underline">Contact us</Link>
          </p>
        </div>
      </div>
    </div>
