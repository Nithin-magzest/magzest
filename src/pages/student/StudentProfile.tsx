...
type AcademicEntry = { id: string; level: string; customLevel: string; institution: string; board: string; year: string; percentage: string; city: string; comment: string; status: string; backlogs: string; attempts: string; };
function newAcademicEntry(): AcademicEntry { return { id: crypto.randomUUID(), level: '10th Grade', customLevel: '', institution: '', board: '', year: '', percentage: '', city: '', comment: '', status: '', backlogs: '', attempts: '' }; }
                      ...
                      <div key={entry.id} className="border border-gray-100 bg-gray-50 rounded-xl p-4">
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1 grid grid-cols-2 sm:grid-cols-3 gap-x-6 gap-y-2 text-sm">
                            ...
                            {entry.status && <div><p className="text-xs text-gray-400">Status</p><p className="font-medium text-gray-800">{entry.status}</p></div>}
